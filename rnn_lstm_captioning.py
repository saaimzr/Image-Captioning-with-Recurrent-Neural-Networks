...
def rnn_step_backward(dnext_h, cache):
    ...
    dS = dnext_h*torch.square(1/torch.cosh(S)) # Shape (N,H)

    dx = Wx.mm(dS.t()).t() # CHECK if you can transpose
    dprev_h = Wh.mm(dS.t()).t()
    dWx = x.t().mm(dS) # CHECK
    dWh = h.t().mm(dS)
    db = torch.sum(dS, dim=0)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_backward(dh, cache):
    ...
    ##########################################################################
    N, T, H = dh.shape
    last_cache = cache[T-1]
    D = last_cache[0].shape[1]
    last_dh = dh[:, T-1, :]

    # Initialise dx
    dx = torch.zeros((N,T,D), device=dh.device, dtype=dh.dtype)

    # Initialise other derivatives
    dx[:,T-1,:], last_dh, dWx, dWh, db = rnn_step_backward(last_dh, last_cache)

    for t in range(T-2, -1, -1):
        last_dh += dh[:, t, :]
        last_cache = cache[t]
        dx[:, t, :], last_dh, dWx_add, dWh_add, db_add = rnn_step_backward(last_dh, last_cache)

        # Add derivatives to reused weights and biases
        dWx += dWx_add
        dWh += dWh_add
        db += db_add
